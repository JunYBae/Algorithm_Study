...
public class Main {
	...
	public static void main(String[] args) throws IOException {
		...
		cost[0][0][0] = 1;
		int answer = simulation(new Point(0, 0, 0));
		System.out.println(answer);

	}
	
	public static int simulation(Point start_p) {
			...
			for (int i = 0; i < size; i++) {
				...
				for (int index = 0; index < 4; index++) {
					...
					if (is_In(x, y)) {
						...
						if (map[x][y] == 1 && point.break_wall < K) {
							if (cost[point.x][point.y][point.break_wall] + 1 < cost[x][y][point.break_wall+1]) {								
								cost[x][y][point.break_wall+1] = cost[point.x][point.y][point.break_wall] + 1;
								queue.add(new Point(x, y, point.break_wall+1));
							}							
						} 
						
						else if (map[x][y] == 0 && cost[point.x][point.y][point.break_wall] + 1 < cost[x][y][point.break_wall]) {
